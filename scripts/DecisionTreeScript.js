...
const loadTrainingData = async () => {
    ...
    if (!file) {
        notie.alert({
            type: 'warning',
            text: 'Por favor selecciona un archivo CSV de entrenamiento.',
            time: 3,
            position: 'top'
        });
        return;
...
const generatechart = () => {
    if (trainingData.length === 0) {
        notie.alert({
            type: 'warning',
            text: 'Por favor carga los datos de entrenamiento antes de generar el árbol.',
            time: 3,
            position: 'top'
        });
        return;
...
document.getElementById('predict').onclick = async () => {
    ...
    const options = {
        ...
        nodes: {
            color: {
                border: '#1E2022', // primary-100
                background: '#C9D6DF', // bg-200
                highlight: {
                    border: '#1E2022', // primary-100
                    background: '#F0F5F9' // bg-100
                },
                hover: {
                    border: '#1E2022', // primary-100
                    background: '#F0F5F9' // bg-100
            ...
            font: {
                color: '#1E2022' // text-100
            }
        },
        edges: {
            color: {
                color: '#788189', // accent-100
                highlight: '#788189', // accent-100
                hover: '#788189' // accent-100
            },
            arrows: {
