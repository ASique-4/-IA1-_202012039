...
const fitPolyModel = async () => {
  if (!polyFileInput.files.length || !polyDegreeInput.value) {
    notie.alert({
      type: 'warning',
      text: 'Por favor selecciona un archivo CSV y el grado del polinomio.',
      time: 3,
      position: 'top'
    });
    return;
  ...
  notie.alert({
    type: 'success',
    text: 'Modelo polinomial ajustado correctamente.',
    time: 3,
    position: 'top'
  });
};
...
const calculatePolyR2 = () => {
  ...
  polyR2Result.textContent = `El Coeficiente R2 del modelo es: ${r2.toFixed(4)}`;
  notie.alert({
    type: 'info',
